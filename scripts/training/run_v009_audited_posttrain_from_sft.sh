...
CPT_CHECKPOINT="${CPT_CHECKPOINT:-${PROJECT}/results/02_training_runs/pretrain_1p2b_v0_0_9_targeted_clean_cpt_initweights_2026-06-15_063552/checkpoint_final}"
...
POSTTRAIN_PACK="${POSTTRAIN_PACK:-${PROJECT}/data/05_datasets/posttrain/v009_regen_posttrain_20260615_0631_audited}"
...
SFT_PREFIX="${SFT_PREFIX:-v0_0_9_audited_14b_cpt_sft}"
DPO_PREFIX="${DPO_PREFIX:-v0_0_9_audited_14b_cpt_sft_dpo}"
GRPO_PREFIX="${GRPO_PREFIX:-v0_0_9_audited_14b_cpt_sft_dpo_grpo_numeric}"

LOG_DIR="${PROJECT}/results/run_logs"
MANIFEST_DIR="${PROJECT}/results/run_manifests"
PIPE_LOG="${PIPE_LOG:-${LOG_DIR}/v009_audited_posttrain_from_sft_${RUN_STAMP}.log}"
GUARD_LOG="${GUARD_LOG:-${LOG_DIR}/v009_audited_posttrain_from_sft_${RUN_STAMP}.memory_guard.log}"
LAUNCH_MANIFEST="${LAUNCH_MANIFEST:-${MANIFEST_DIR}/v009_audited_posttrain_from_sft_${RUN_STAMP}.launch.json}"
...
if [[ "${FIRST_VISIBLE_GPU}" =~ ^[0-9]+$ ]]; then
  ...
  if [ "${GPU_FREE_MIB}" -lt "${MIN_GPU_FREE_MIB}" ]; then
    echo "selected GPU does not have enough free memory for the default v0.0.9 SFT/DPO/GRPO settings." >&2
    exit 4
  fi
else
...
{
  "run_stamp": "${RUN_STAMP}",
  "stage": "v0.0.9_audited_posttrain_from_completed_cpt",
  "project": "${PROJECT}",
  ...
  "method": "resume completed CPT checkpoint -> SFT -> target_eval -> DPO -> target_eval -> optional GRPO numeric -> target_eval"
}
JSON
...
run_target_eval() {
  ...
  local out_dir="${EVAL_ROOT}/v0_0_9_audited_${stage}_target_eval_${RUN_STAMP}"
  local extra_args=()
...
echo "stage=cpt status=already_completed checkpoint=${CPT_CHECKPOINT}"
...
"${MEMORY_GUARD}" wrap -- \
  "${PYTHON}" -u scripts/train_tiny_reasoning_sft.py \
    ...
    --run-type v0_0_9_audited_cpt_sft \
    --seed 42 \
    "${SFT_EXTRA_ARGS[@]}"
...
"${MEMORY_GUARD}" wrap -- \
  "${PYTHON}" -u scripts/train_reasoning_dpo.py \
    ...
    --run-type v0_0_9_audited_cpt_sft_dpo \
    --seed 42 \
    "${DPO_EXTRA_ARGS[@]}"
...
if [ "${RUN_GRPO_NUMERIC}" = "1" ]; then
  ...
  "${MEMORY_GUARD}" wrap -- \
    "${PYTHON}" -u scripts/train_grpo_numeric_trl.py \
      ...
      --run-type v0_0_9_audited_cpt_sft_dpo_grpo_numeric_probe \
...
PIPELINE_SUMMARY="${OUTPUT_ROOT}/v0_0_9_audited_posttrain_from_sft_${RUN_STAMP}.summary.json"
